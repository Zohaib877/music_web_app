...
} from "@/lib/features/PlayList/createPlayList";
import { AppDispatch, RootState } from "@/lib/store";
import { useDispatch, useSelector } from "react-redux";
import { closePlaylistModel } from "@/lib/features/PlayList/playListModal";

interface AddToPlayListProp {
  is_playlist: boolean;
}

const AddToPlayListModal: React.FC<AddToPlayListProp> = ({ is_playlist }) => {
    ...
    >
      <div className="relative dark:bg-gray-700 w-full max-w-md p-6 rounded-lg shadow-lg mx-4">
        <div className="flex items-center justify-between md:p-5 border-b rounded-t dark:border-gray-600">
          ...
          <button
            type="button"
            onClick={() => dispatch(closePlaylistModel())}
              ...
              {playlists.map((item) => (
                <li key={item.id} className="mx-3">
                  <button
                    ...
                    className="flex items-center p-3 w-full text-base font-semibold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 dark:bg-gray-600 dark:text-white dark:hover:bg-gray-500"
                  >
                    {item.name}
                  </button>
                </li>
